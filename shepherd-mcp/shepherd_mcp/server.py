...
@mcp.tool()
def drone_files(job_id: str) -> str:
    """
    Return the content of all files generated by a drone job.

    Reads files directly from the job's worktree — use this to review generated
    code after drone_wait returns 'ready', instead of navigating to the worktree
    path manually.

    Returns JSON: {"job_id": str, "files": {relative_path: content}}
    Returns {"error": str} if the job is unknown or has no worktree.
    """
    try:
        job = store.get(job_id)
    except KeyError:
    ...
    if not job.worktree_path:
        return json.dumps({"error": "Job has no worktree."})

    root = Path(job.worktree_path)
    ...
    for rel_path in job.files:
        ...
        if file_path.exists():
            ...
            contents[rel_path] = f"error: file not found: {rel_path}"

    return json.dumps({"job_id": job_id, "files": contents}, indent=2)
